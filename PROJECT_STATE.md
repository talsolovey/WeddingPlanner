...
| Step 27 | Eval-driven skill iteration (the loop, demonstrated with numbers): added explicit row-audit sections to `budget-forecaster` (duplicates, overdue payments, per-line overruns, mandatory budgeted-sum-vs-total statement, exhaustive missing-category sweep) and `guest-list-manager` (impossible counts, confirmed-but-zero, "check EVERY household"; capacity yellow/red always reported). Re-runs: budget **2/6 → 4/6**, guests **3/5 → 4/5** (median of repeat runs). Two honest residuals: (1) single-run recall varies ±1 — evals should be repeated and read as medians; (2) `totals-exceed` (summing 7 line items) never lands across runs — model arithmetic again; the right fix is the `weeks_to_wedding` pattern: compute sums in code and feed them as computed_facts (backlogged) | ✅ before/after results in `evals/results/`; ~$0.10 of iteration runs |
| Step 28 | Computed facts for specialists (closing the eval finding): `compute_facts()` in the orchestrator does the arithmetic models fail at — budget line-sums vs total (+overage), duplicate lines, per-line overruns, overdue unpaid; guest attending/party-size totals vs capacity; per-table seating loads — computed once in code and injected into every specialist AND verifier prompt as trusted numbers ("trust these over your own arithmetic"). Evals measure the same path. Result: the never-caught `totals-exceed` trap now lands; **guest-list-manager 5/5** (first perfect run); budget stable at 4/6 with only soft-judgment misses rotating | ✅ 4 new offline tests (sums/dups/overdue/loads + facts-reach-prompts); full suite green |
| Step 29 | Robustness pass (all five audit findings): (1) **CI** — GitHub Actions runs the offline suite + eval dry-run on every push; (2) **error visibility** — global handler: unhandled exceptions log with traceback and return JSON on /api/* (never Flask's HTML 500), HTTP errors on /api/* are JSON too; (3) **bounded memory** — finished background jobs pruned past 200, stale rate-limiter IPs swept; (4) **atomic writes** — `storage.mutate()` (per-couple-per-document lock, load→fn→save); wave delivery rewritten to mutate only its own wave per recipient, so concurrent edits to other waves survive a paced delivery (test proves it); (5) **frontend** — `VOW.getJSON/postJSON` helpers (throw on non-2xx, 401→login) and all 26 previously-unchecked fetches across 10 pages converted, failures now toast instead of silently rendering `undefined` | ✅ 176 tests pass ×3 (10 new robustness tests incl. concurrent-increment and delivery-vs-edit races) |
| Step 30 | Safety-audit fixes (four injection channels closed, each tested): name fields (household/group/budget vendor/category) scanned at write time — identity strings enter agent prompts; `append_lesson` scans lessons before recording (agent memory = persistent injection surface); chat snapshot carries an explicit data-not-commands fence over interpolated names/notes/contract flags; due waves with a BLANK message never auto-deliver (the default template is never sent without the couple having seen it — manual send with visible preview still works). SECURITY.md updated (post-audit section + CSRF/SameSite note + credential-rotation checklist) | ✅ 184 tests pass ×2 (8 new in `test_injection_gaps.py`) |
