...
- **Upload a contract → red-flag report.** The agent reads the PDF against a learned checklist (jurisdiction traps, auto-renewals, missing contingencies) and returns severity-ranked flags. Live at `/contracts`. ▶ [55-sec narrated clip](./evidence/demos/3-contracts.mp4) — a real (Hebrew!) venue contract analyzed end to end.
- **"Refresh my brief" → a specialist team investigates.** Four sub-agents (contracts / budget / guests / logistics) fan out in parallel, a verifier re-checks each one, and the couple gets a ranked to-do list on the home dashboard. The magic moment: the verifier badge — *"caught 5 items the specialists missed"* — visible in the UI. ▶ [55-sec narrated clip](./evidence/demos/1-weekly-brief.mp4) — sign-in to fresh brief, specialists + verifier live.
- **Silent guests → Vow proposes the nudge.** RSVP deadline approaching + households silent → Vow drafts a personalized WhatsApp reminder (magic RSVP link included) and posts it as an *approvable proposal* on home. Approve → it sends via Twilio and later reports whether the nudge actually produced a reply. This flow has run for real: WhatsApp nudges were delivered to actual guests through the live app and the replies came back through the magic RSVP links (the demo account's activity log shows the sends). ▶ [46-sec narrated clip](./evidence/demos/2-guests-rsvp-seating.mp4) — a guest replies through their magic link, the headcount updates, and the agent seats the room with code-validated conflicts.
...
**The threat we take seriously:** an incumbent (Zola, The Knot) could bolt an agent onto their marketplace within quarters. Two structural answers, both already scaffolded: (1) a **cross-wedding learning corpus** — the lessons loop generalized into vendor-level knowledge ("this venue's contracts have hidden a rebooking fee in 4 of 5 weddings we've seen"), aggregated across ≥3 couples with no couple data ever crossing tenants; it compounds only with usage, and the incumbents' own marketplace incentives (vendors are their *paying customers*) make warning couples about those vendors structurally awkward for them. (2) **Guest-side distribution** — every wedding puts Vow's RSVP links and WhatsApp messages in front of ~200 guests, a demographic dense with soon-to-be-engaged couples; each wedding markets the next at near-zero CAC.

**Beachhead: Israel.** The US incumbents don't operate there — no The Knot, no Zola — so the agent-first planner isn't a differentiated feature, it's the first real product in the category. The market fits Vow's design unusually well: weddings are large (300–500 guests is normal, multiplying both the coordination pain and the guest-side distribution per wedding) and coordination is already WhatsApp-native — exactly the channel Vow sends on. The product is quietly built for it: phone normalization defaults to +972, and the contracts demo clip is a real Hebrew venue contract analyzed end to end. Next milestone: a beta season with real couples, measuring replies-per-nudge and brief items acted on.
...
- **Demo clips (narrated):** [`./evidence/demos/`](./evidence/demos/) — three ~1-minute recordings of the live app: the specialist team + verifier producing a brief, a guest RSVP round-trip + agent seating, and a real contract analysis.
